...
if [ -z "${snell_port}" ]; then
    while true; do
        ...
        # 预先检查端口是否可用 (需要 ss 命令，后续会安装)
        if ! type ss > /dev/null 2>&1 || ! ss -lntu | grep -q ":${snell_port} "; then
...
echo "============================================="
echo "安装将在 2 秒后开始，按 Ctrl+C 取消"
sleep 2

# --- 步骤 3: 安装与部署 ---

echo "--> (1/2) 更新软件包列表并安装必要工具 (nano, wget, unzip, curl)..."
apt-get update > /dev/null
apt-get install -y nano wget unzip curl > /dev/null
if [ $? -ne 0 ]; then
...
echo "--> (2/2) 正在下载、解压并部署 Snell 服务器程序..."
